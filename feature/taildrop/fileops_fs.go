...
func (d fsFileOps) OpenWriter(name string, offset int64, perm os.FileMode) (io.WriteCloser, string, error) {
	path, err := joinDir(d.rootDir, name)
	if err != nil {
		return nil, "", err
	}
...
func (d fsFileOps) Remove(name string) error {
	path, err := joinDir(d.rootDir, name)
	if err != nil {
		return err
	}
...
func (m fsFileOps) Rename(oldPath, newName string) (newPath string, err error) {
	var dst string
	if filepath.IsAbs(newName) || strings.ContainsRune(newName, os.PathSeparator) {
		dst = newName
	} else {
		dst, err = joinDirForUpload(m.rootDir, newName)
		if err != nil {
			return "", err
		}
	}
	...
	for i := 0; i < maxRetries; i++ {
		...
		dst, err = joinDirForUpload(filepath.Dir(dst), nextFilename(filepath.Base(dst)))
		if err != nil {
			return "", fmt.Errorf("generating retry filename: %w", err)
		}
	}
...
func (d fsFileOps) ListFiles() ([]string, error) {
...
func (d fsFileOps) Stat(name string) (fs.FileInfo, error) {
	path, err := joinDir(d.rootDir, name)
	if err != nil {
		return nil, err
	}
...
func (d fsFileOps) OpenReader(name string) (io.ReadCloser, error) {
	path, err := joinDir(d.rootDir, name)
	if err != nil {
		return nil, err
	}
...
// joinDirForUpload allows reserved suffixes like ".partial"
// and ".deleted". This is used when resolving paths that may
// include such suffixes (eg, during upload finalization, recovery, or cleanup.)
func joinDirForUpload(dir, baseName string) (string, error) {
	return filepath.Join(dir, baseName), nil
}
